...
export default function PdfUploader({
  onProcessingStart,
  disableUpload = false,
}: PdfUploaderProps & { disableUpload?: boolean }) {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setError(null);
    console.log(
      'PdfUploader: Starting upload for file:',
      selectedFile.name,
      'by user:',
      session.user.id
    );
    ...
    try {
      ...
      console.log(
        'PdfUploader: Sending POST request to:',
        `${backendUrl}/upload-and-process-pdf/`
      );
      ...
      console.log(
        'PdfUploader: Received response from backend. Status:',
        response.status
      );
      ...
      if (!response.ok) {
        ...
        try {
          ...
          console.error(
            'PdfUploader: Could not parse JSON from error response. Response text:',
  ...
  return (
    ...
    >
      ...
      >
        ...
        {error && (
          <p className='text-sm text-red-500 bg-red-800/10 p-3 rounded-md'>
            {error}
          </p>
        )}
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.98 }}
          disabled={isUploading || !selectedFile || disableUpload}
          className={`
